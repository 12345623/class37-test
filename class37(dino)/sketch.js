...
function setup() {
  ...
  invisiground=createSprite(200,195,9000,10);
...
function draw() {
  ...
  if (ground.x<100)
   ...
   if (invisiground.x=9000){
   invisiground.x=invisiground+9000;
   }
  ...
  if (gamestate===play){
   if (keyDown("space")&&trex.y>160){
    ...
    if(frameCount%5===0)
    score=score+1;
    text("SCORE "+score,trex.x,50,textSize(15))
...
function spawnclouds() {
  if (frameCount%50===0){
  var cloud=createSprite (trex.x+400,Math.round (random (1,160) ))
...
function spawnobstacles(){
  if (frameCount%60===0){
  var obstacle=createSprite(trex.x+400,170)
  obstacle.scale=0.6;
